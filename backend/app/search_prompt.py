...
명세서는 청구항과 같은 칸에 넣지 않는다. 두 자료는 역할이 다르다.
    ...
    청구항   검색 범위를 정하는 기준.
    명세서   청구항 용어를 해석하고 검색어를 넓히는 참고 자료. 범위를 좁히는
             근거가 아니다.

검색은 한 번만 실행하므로 명세서도 청구항과 같은 모델 호출에 들어간다. 청구항
단독 실행과 명세서 보조 실행을 따로 돌리던 방식은 88efc5b 에서 없앴다. 그래서
명세서가 검색 범위를 좁히지 못하게 막는 것은 실행 분리가 아니라
search_contract.SPEC_PREAMBLE 의 사용 규칙이다. 여기서는 자료 역할이 섞이지 않게
``<SPEC_TEXT>`` … ``</SPEC_TEXT>`` 경계를 둔다.
