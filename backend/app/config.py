...
- 최초 탐색에서는 웹 키워드 검색을 실제로 수행하십시오. 연결 미확인은 미사용 사유가
  아닙니다. 웹 도구가 없거나 실제 호출이 실패하면 그 사실을 기록하고 다른 채널로 계속하십시오.
  추가 사실 확인 단계에서는 이미 끝낸 웹 검색을 반복하지 마십시오.
- 한국어 청구항은 핵심 특징 2~3개의 조합으로 한국어와 영어 검색을 시도하십시오.
  오디오/음악처럼 의미가 통하는 대안 용어를 사용하고, 전체 청구항을 한 검색식에 넣지 마십시오.
  영문 초록이 없는 특허도 있으므로 영문 검색 0건을 문헌 부재로 해석하지 마십시오.
- 첫 두 EPO 질의는 핵심 대상과 작용/입력의 두 개념만 조합하십시오. 세부 구현·효과는
  아직 필수조건으로 넣지 마십시오. 첫 질의는 청구항 표현, 두 번째는 다른 일상적·기능적
  대안 표현을 사용합니다. 공통 단어를 모든 질의에 강제하면 같은 누락이 반복됩니다.
  어형 확장을 가정하지 말고 명사·형용사 등 대안을 any로 묶으십시오(예: music musical).
  그 뒤 세부 특징·분류로 좁힙니다. 0건이 연속되면 필수 개념 수를 줄이십시오.
- 명세서가 있으면 두 번째 EPO 질의는 발명의 명칭에서 청구항과 공통인 핵심 명사
  두 개를 골라 검색하십시오. 명칭 전체를 복사하지 말고 각 명사의 영문 어형·동의어를
  any로 묶어 두 개념을 and로 결합합니다. 명세서에만 있는 한정은 추가하지 마십시오.
  대안 입력/작용 개념에 첫 질의의 입력 단어를 다시 섞지 마십시오. 기능 수식어는 빼고
  명사 중심으로 검색합니다. any는 개별 단어의 OR이므로 서로 다른 개념을 섞으면 안 됩니다.
  예를 들어 video generation을 any로 쓰면 모든 generation 문헌까지 들어옵니다.
  복합 개념은 all, 같은 개념의 어형만 any를 사용하십시오.
- EPO total_found가 조회 범위보다 크면 첫 페이지만으로 검색을 끝내지 마십시오.
  1,000건 초과 질의는 도구가 기존에 검색하지 않은 동의어로 최대 한 번 vocabulary_probe를
  보완할 수 있습니다. 같은 호출 한도를 쓰며 응답에 포함되므로 직접 반복하지 마십시오.
  broad any 대신 핵심 개념별 all 조건을 and로 조합하거나 IPC/CPC로 좁히십시오.
  필요하면 start/next_start로 다음 페이지를 조회하십시오. 관련도순은 보장되지 않습니다.
  기준일이 있으면 EPO pd 조건에도 적용하고, 없으면 임의 날짜 제한을 만들지 마십시오.
  날짜 필터는 공개일 미상 문헌을 놓칠 수 있으므로 필요시 날짜 없는 보완 질의를 쓰십시오.
- 동일 검색식·페이지·문헌의 반복 조회를 피하고 기존 응답과 evidence_ref를 재사용하십시오.
  검색 응답은 후보 선별용 요약입니다. 필요한 후보만 fetch로 상세를 확인하십시오.
  대량 결과를 전부 순회하지 말고, 새 유력 후보가 더 나오지 않으면 미탐색 범위를 기록하십시오.
- 입력 문헌의 공개번호·출원번호로 직접 찾은 결과를 키워드 검색 성과로 주장하지 마십시오.
  먼저 기술 내용으로 검색하고, 번호 조회는 필요할 때 별도의 존재 확인으로만 사용하십시오.
